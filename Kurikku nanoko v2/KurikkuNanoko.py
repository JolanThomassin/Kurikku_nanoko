...
class Jeux() :
	...
	def LancerCredit(self):
		...
		self.texteBéta1 = self.CanvaUn.create_text(self.LargeurEcran/2, 373, text= "Johan Baton", font=("OCR A Extended", 45), fill="#FF14E6")
		self.texteBéta1Double = self.CanvaUn.create_text(self.LargeurEcran/2, 370, text= "Johan Baton", font=("OCR A Extended", 45), fill="#FBFF0F")
		...
		self.texteBéta3 = self.CanvaUn.create_text(self.LargeurEcran/2, 473, text= "", font=("OCR A Extended", 45), fill="#FF14E6")
		self.texteBéta3Double = self.CanvaUn.create_text(self.LargeurEcran/2, 470, text= "", font=("OCR A Extended", 45), fill="#FBFF0F")
	...
	def FenetrePrincipale(self):
		...
		### Boutons ###
		self.boutonCodeClikable = Button(self.EcranDeJeux, text="Code", font=("OCR A Extended", 20), bg='white', fg='black', command=self.pageCode)
		self.boutonCodeClikable = self.CanvaUn.create_window(720, 590, window=self.boutonCodeClikable)
	...
	def ChargerSauvegarde(self):
		with open("sauvegarde.txt", "r") as fichier :
			lignes = fichier.readlines()
			self.argent = float(lignes[0])
			self.argentTotal = float(lignes[1])
	...
	def pageCode(self):
		...
		def récupérationValeur():
			...
			if (code == "XBD") and (self.waifuCode == False) :
				self.CanvaUn.itemconfigure(self.résultaCode, text="Vous avez débloqué : Chloé")
				self.waifuCode = True
			if (code == "Jolan") :
				self.CanvaUn.itemconfigure(self.résultaCode, text="Il est magnifique")
			if (code == "Johan") :
				self.CanvaUn.itemconfigure(self.résultaCode, text="Il est... spécial")
			if (code == "Mathis") :
				self.CanvaUn.itemconfigure(self.résultaCode, text="Il est dépréssif")
			if (code == "Mehdi") :
				self.CanvaUn.itemconfigure(self.résultaCode, text="Il est pas drole")
			if (code == "Nicolas") :
				self.CanvaUn.itemconfigure(self.résultaCode, text="Il te déboite en 1vs1 Isaac")
			if (code == "Chloé") :
				self.CanvaUn.itemconfigure(self.résultaCode, text="Elle est belle")
		...
		### Boutons ###
		self.boutonRetour = Button(self.EcranDeJeux, text=" Retour ", font=("OCR A Extended", 20), width=20, bg='white', fg='black', command=self.FenetrePrincipale)
		self.boutonRetour = self.CanvaUn.create_window(self.LargeurEcran/2, 680, window=self.boutonRetour)
	...
	def debitArgentCaisse(self):
		if self.argent >= self.prixCaisse :
			...
			self.prixCaisse = self.prixCaisse * 1.2
			self.ActualisationArgent()
			self.tirageCaisse()
		else :
			None	
	...
	def pageWaifu(self):
		...
		self.descriptionWaifu = self.CanvaUn.create_text((self.LargeurEcran/2)+223, (self.HauteurEcran/2)-103, text= "", font=("OCR A Extended", 30), fill="white")
		self.descriptionWaifuDouble = self.CanvaUn.create_text((self.LargeurEcran/2)+220, (self.HauteurEcran/2)-100, text= "", font=("OCR A Extended", 30), fill="black")
		self.prixWaifu = self.CanvaUn.create_text((self.LargeurEcran/2)+233, (self.HauteurEcran/2)+203, text= "", font=("OCR A Extended", 30), fill="white")
		self.prixWaifuDouuble = self.CanvaUn.create_text((self.LargeurEcran/2)+230, (self.HauteurEcran/2)+200, text= "", font=("OCR A Extended", 30), fill="black")
	...
	def pageWaifuUn(self):
		...
		self.CanvaUn.delete(self.texteNomWaifuDouble)
		self.CanvaUn.delete(self.texteNomWaifu)
		self.CanvaUn.delete(self.descriptionWaifu)
		self.CanvaUn.delete(self.descriptionWaifuDouble)
		self.CanvaUn.delete(self.prixWaifu)
		self.CanvaUn.delete(self.prixWaifuDouuble)
		...
		if self.waifuUn == False :
			...
			self.boutonWaifuUn = self.CanvaUn.create_window(self.LargeurEcran/2-250, self.HauteurEcran/2, window=self.boutonWaifuUn)
		self.boutonGauche = Button(self.EcranDeJeux, text="←", font=("OCR A Extended", 20), bg='white', fg='black', command=self.pagWaifuCode)
		self.boutonGauche = self.CanvaUn.create_window(self.LargeurEcran/2-500, self.HauteurEcran/2, window=self.boutonGauche)
		self.boutonDroite = Button(self.EcranDeJeux, text="→", font=("OCR A Extended", 20), bg='white', fg='black', command=self.pagWaifuCode)
		self.boutonDroite = self.CanvaUn.create_window(self.LargeurEcran/2+500, self.HauteurEcran/2, window=self.boutonDroite)
		...
		self.descriptionWaifuDouble = self.CanvaUn.create_text((self.LargeurEcran/2)+220, (self.HauteurEcran/2)-100, text= "Augmentera la puissance\n     de vos cliques", font=("OCR A Extended", 30), fill="black")
		if self.waifuUn == False :
	...
	def pagWaifuCode(self):
		...
		self.descriptionWaifu = self.CanvaUn.create_text((self.LargeurEcran/2)+223, (self.HauteurEcran/2)-103, text= "Pouvoir inconnu (pour le moment)", font=("OCR A Extended", 30), fill="white")
		self.descriptionWaifuDouble = self.CanvaUn.create_text((self.LargeurEcran/2)+220, (self.HauteurEcran/2)-100, text= "Pouvoir inconnu (pour le moment)", font=("OCR A Extended", 30), fill="black")
		if self.waifuCode == False :
	...
	def pageBoutonSecret(self):
		...
		### Boutons ###
		self.boutonMenuQuitter = Button(self.EcranDeJeux, text="  Retour  ", font=("OCR A Extended", 20), bg='white', fg='#A44040', command=self.pageWaifu)
		self.boutonMenuQuitter = self.CanvaUn.create_window(self.LargeurEcran/2, self.LargeurEcran/1.70, window=self.boutonMenuQuitter)
	...
	def Ressources(self):
		...
		self.boutonRetour = Button(self.EcranDeJeux, text=" Retour ", font=("OCR A Extended", 20), bg='white', fg='black', command=self.MenuBoutique)
		self.boutonRetour = self.CanvaUn.create_window(self.LargeurEcran/2, 680, window=self.boutonRetour)
		self.boutonAchatWeebs = Button(self.EcranDeJeux, text=" Achat de 1 Weeb pour : "  + str(self.prixWeeb) + "€", font=("OCR A Extended", 20), bg='white', fg='black', command=self.AchatWeebUn)
	...
	def RessourcesFois10(self):
		...
		### Variables ###
		prix10 = self.suiteAchatWeeb(8)

		### Boutons ###
		self.boutonAchatWeebs = Button(self.EcranDeJeux, text=" Achat de 10 weebs pour : "  + str(prix10) + "€", font=("OCR A Extended", 20), bg='white', fg='black', command=self.AchatWeebDix)
		self.boutonAchatWeebs = self.CanvaUn.create_window(self.LargeurEcran/2, 300, window=self.boutonAchatWeebs)
	...
	def RessourcesFois100(self):
		...
		### Variables ###
		prix100 = self.suiteAchatWeeb(98)

		### Boutons ###
		self.boutonAchatWeebs = Button(self.EcranDeJeux, text=" Achat de 100 weebs pour : "  + str(prix100) + "€", font=("OCR A Extended", 20), bg='white', fg='black', command=self.AchatWeebCent)
		self.boutonAchatWeebs = self.CanvaUn.create_window(self.LargeurEcran/2, 300, window=self.boutonAchatWeebs)
		self.boutonFoisUn = Button(self.EcranDeJeux, text="x100", font=("OCR A Extended", 20), bg='white', fg='black', command=self.RessourcesMAXIMUM)
		self.boutonFoisUn = self.CanvaUn.create_window(self.LargeurEcran/2, 200, window=self.boutonFoisUn)

	def RessourcesMAXIMUM(self):
		### Reset ###
		self.CanvaUn.delete(self.boutonFoisUn)
		self.CanvaUn.delete(self.boutonAchatWeebs)

		### Variables ###
		prixMax = self.prixWeebMAXIMUM()
		weebMaxAchetable = self.nombreWeebMAXIMUM()

		### Boutons ###
		self.boutonAchatWeebs = Button(self.EcranDeJeux, text=" Achat de " + str(weebMaxAchetable) + " weeb (1000€/5 Seconde) pour : " + str(prixMax) + "€", font=("OCR A Extended", 20), bg='white', fg='black', command=self.AchatWeebMAXIMUM)
		self.boutonAchatWeebs = self.CanvaUn.create_window(self.LargeurEcran/2, 300, window=self.boutonAchatWeebs)
		self.boutonFoisUn = Button(self.EcranDeJeux, text="xMAX", font=("OCR A Extended", 20), bg='white', fg='black', command=self.Ressources)
		self.boutonFoisUn = self.CanvaUn.create_window(self.LargeurEcran/2, 200, window=self.boutonFoisUn)

	def transformationDécimal(self):
		rés = "%.2f" % self.prixWeeb
		self.prixWeeb = float(rés)
	...
	def AchatWeebUn(self):
		if self.argent >= self.prixWeeb :
			...
			self.prixWeeb = self.prixWeeb * 1.07
			self.transformationDécimal()
			self.ActualisationArgent()
			self.boutonAchatWeebs = Button(self.EcranDeJeux, text=" Achat weeb (1€/5 Seconde) pour : "  + str(self.prixWeeb) + "€", font=("OCR A Extended", 20), bg='white', fg='black', command=self.AchatWeebUn)
			self.boutonAchatWeebs = self.CanvaUn.create_window(self.LargeurEcran/2, 300, window=self.boutonAchatWeebs)
	def AchatWeebDix(self):
		prix10 = self.suiteAchatWeeb(8)
		if self.argent >= prix10 :
			self.CanvaUn.delete(self.boutonAchatWeebs)
			self.argent = self.argent - prix10
			self.weeb = self.weeb + 10
			self.augmentationAchatWeeb(8)
			self.ActualisationArgent()
			prix10 = self.suiteAchatWeeb(8)
			self.boutonAchatWeebs = Button(self.EcranDeJeux, text=" Achat weeb (10€/5 Seconde) pour : "  + str(prix10) + "€", font=("OCR A Extended", 20), bg='white', fg='black', command=self.AchatWeebDix)
			self.boutonAchatWeebs = self.CanvaUn.create_window(self.LargeurEcran/2, 300, window=self.boutonAchatWeebs)
	def AchatWeebCent(self):
		prix100 = self.suiteAchatWeeb(98)
		if self.argent >= prix100 :
			self.CanvaUn.delete(self.boutonAchatWeebs)
			self.argent = self.argent - prix100
			self.weeb = self.weeb + 10
			self.augmentationAchatWeeb(98)
			self.ActualisationArgent()
			prix100 = self.suiteAchatWeeb(98)
			self.boutonAchatWeebs = Button(self.EcranDeJeux, text=" Achat weeb (10€/5 Seconde) pour : "  + str(prix100) + "€", font=("OCR A Extended", 20), bg='white', fg='black', command=self.AchatWeebDix)
			self.boutonAchatWeebs = self.CanvaUn.create_window(self.LargeurEcran/2, 300, window=self.boutonAchatWeebs)
	def AchatWeebMAXIMUM(self):
		### Variables ###
		prixMax = self.prixWeebMAXIMUM()
		weebMaxAchetable = self.nombreWeebMAXIMUM()
		if self.argent >= prixMax :
			self.CanvaUn.delete(self.boutonAchatWeebs)
			self.argent = self.argent - prixMax
			self.weeb = self.weeb + weebMaxAchetable
			self.augmentationAchatWeeb((weebMaxAchetable)-2)
			self.ActualisationArgent()
			prixMax = self.prixWeebMAXIMUM()
			self.boutonAchatWeebs = Button(self.EcranDeJeux, text=" Achat weeb (10€/5 Seconde) pour : "  + str(prixMax) + "€", font=("OCR A Extended", 20), bg='white', fg='black', command=self.AchatWeebDix)
			self.boutonAchatWeebs = self.CanvaUn.create_window(self.LargeurEcran/2, 300, window=self.boutonAchatWeebs)
	def suiteAchatWeeb(self, n):
		up = self.prixWeeb
		u = (up * 1.07)
		somme = up + u
		for i in range(n):
		...
		somme = float(rés)
		return(somme)
	def augmentationAchatWeeb(self, n):
		up = self.prixWeeb
		u = (up * 1.07)
		somme = up + u
		for i in range(n):
	...
	def argentDécimal(self):
		rés = "%.2f" % self.argent
		self.argent = float(rés)

	def prixWeebMAXIMUM(self):
		stockage = 0
		up = self.prixWeeb
		u = (up * 1.07)
		somme = up + u
		while somme <= self.argent :
			u = (u * 1.07)
			rés = "%.2f" % u
			u = float(rés)
			stockage = somme
	...
	def FonctionTimer(self):
		self.argent = self.argent + (self.weeb/5)
		rés = "%.2f" %(self.argentTotal + (self.weeb/5))
		self.argentTotal = float(rés)
		### Textes ###
		if self.InGame == True :
			self.ActualisationArgent()
		self.CanvaUn.after(1000, self.FonctionTimer)
